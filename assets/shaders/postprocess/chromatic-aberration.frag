...
void main(){
    ...
    vec4 left_color = texture(tex, vec2(tex_coord.x - STRENGTH, tex_coord.y));
    vec4 right_color = texture(tex, vec2(tex_coord.x + STRENGTH, tex_coord.y));
 
    frag_color = vec4(left_color.r, frag_color.g, right_color.b, frag_color.a);
}